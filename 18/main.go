...
// 	dsu := NewDSU(total)
// 	open := make([]bool, total)

// 	toID := func(x, y int) int {
// 		return y*width + x
// 	}

// 	// Build DSU for the final state after all bytes have fallen.
// 	for y := 0; y < height; y++ {
// 		for x := 0; x < width; x++ {
// 			if new_matrix[y][x] == "." {
// 				id := toID(x, y)
...
// 	startID := toID(0, 0)
// 	endID := toID(max_x, max_y)

// 	for i := len(bytes) - 1; i >= 0; i-- {
// 		x, y := parseCoords(bytes[i])
// 		id := toID(x, y)
// 		if open[id] {
// 			continue
// 		}
...
// 		if open[startID] && open[endID] && dsu.Connected(startID, endID) {
// 			return bytes[i]
// 		}
// 	}
